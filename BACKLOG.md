...
## v0.3 (post-MCP)
- [⏳] Multi-step agent loop: Claude calls tool, sees result, decides next tool — with per-call y/N gates *(slice 1: `--mcp-agent` + per-turn y/N + hard cap; slice 2: consecutive-error abort with exit 8; parallel tool calls and self-reflection deferred)*
