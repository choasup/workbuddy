...
## Acceptance
- [x] `src/workbuddy/cli.py` adds a new argparse arg `--mcp-agent-dry-run` (boolean, default False). Help: `When combined with --mcp-agent: simulate the loop without executing any tool. Each turn prints "DRY RUN: would call ..." and feeds Claude a synthetic success result so the plan continues. No y/N prompts, no real tool invocations`
- [x] Validation in `main()` BEFORE the existing dispatch:
...
- [x] In `_run_mcp_agent`, after the cold-rejection checks (multi-tool / hallucination / non-dict input still fire per turn — DRY-RUN MUST NOT BYPASS THEM) but at the point where the live mode would do `print(f"Turn {turn_index + 1}/{max_turns}", ...)` plus the y/N prompt:
...
- [x] DRY-RUN MUST NOT BYPASS any cold-rejection. Multi-tool / hallucination / non-dict-input rejections still happen with the same exit codes (5) and history records (`mcp_rejection_reason` still recorded). The dry-run ONLY changes what happens AFTER the proposed call passes those checks
- [x] DRY-RUN does NOT change `final-text` exit semantics — if Claude returns text-only, dry-run still prints it and exits 0
- [x] DRY-RUN does NOT change `max-turns-reached` semantics — if the loop hits `max_turns` of synthetic-success turns without a final-text, exit 7 with `max-turns-reached` history record
- [x] `tests/test_cli.py` adds these tests:
...
- [x] All 94 existing tests must continue to pass unchanged. The slice 1+2 tests do NOT use dry-run, so their behaviour is byte-identical
- [x] `python -m pytest -W error` passes
- [x] `README.md` `--mcp-agent` paragraph: append a final sentence (or new paragraph) — `Use --mcp-agent-dry-run to inspect the plan without executing: each turn prints "DRY RUN: would call ..." and Claude is fed a synthetic success result so the plan can continue. No y/N prompts, no real tool invocations.`
- [x] BACKLOG: extend the v0.3 annotation to `*(slice 1: --mcp-agent + per-turn y/N + hard cap; slice 2: consecutive-error abort; slice 3: --mcp-agent-dry-run; parallel tool calls and self-reflection deferred)*`
- [x] Total project Python LOC stays under ~3100
