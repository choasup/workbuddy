...
- [x] Add `--mcp-call-tool` to the existing `mode_group` (mutex with `--exec`, `--git`, `--mcp-list-tools`). Verified by adding two more mutex tests
- [x] In `main()`, BEFORE the API key check, after the existing `--mcp-list-tools`/`--mcp-server` validation:
...
- [x] Add async helper `async def _async_call_tool(server_argv, tool_name, tool_args)`:
...
- [x] Add sync wrapper `def _run_mcp_call_tool(args) -> int`:
...
- [x] CRITICAL safety property: validation errors (bad JSON, missing arg pair, non-dict args) MUST short-circuit BEFORE the y/N prompt is shown. Otherwise the user might confirm a malformed call. Test this with the `_input_must_not_be_called` sentinel
- [x] CRITICAL UX property: y/N prompt is strict — `{"y", "Y"}` only. Empty input / EOF / `yes` / `Y\n` (the latter handled by `.strip()`) abort. Same posture as `--exec` / `--git`
- [x] `tests/test_cli.py` adds these tests using `monkeypatch.setattr(cli_mod, "_async_call_tool", _fake)`:
...
- [x] All 58 existing tests must continue to pass unchanged
- [x] `python -m pytest -W error` passes (no resource warnings — same standard as slice 1)
- [x] `README.md` "Usage" section gains a `--mcp-call-tool` paragraph after the `--mcp-list-tools` paragraph: example `workbuddy --mcp-call-tool echo --mcp-tool-args '{"text": "hi"}' --mcp-server "python -m my_server" .`. Note that args must be a JSON object, the call requires y/N confirmation, exit codes match other modes (0 success, 5 error / tool-isError, 6 timeout), and a future slice will add Claude-driven tool selection
- [x] Update the BACKLOG annotation: change `(slice 1: --mcp-list-tools listing only; tool execution and resource subscription deferred)` to `(slice 1+2 shipped: --mcp-list-tools and --mcp-call-tool with user-provided args; slice 3 will add Claude-driven tool selection)`. Keep the line at `[⏳]` since slice 3 still pending
- [x] Total project Python LOC stays under ~1700
