...
## Acceptance
- [ ] Import the helpers directly: `from workbuddy.cli import _extract_text, _log_run, MAX_LOGGED_RESPONSE_CHARS`
- [ ] Add a test for `_extract_text` covering at minimum these shapes (use `types.SimpleNamespace` to fake the response and content blocks):
...
- [ ] Add a test for `_log_run` truncation: write a string of length `MAX_LOGGED_RESPONSE_CHARS + 100`, call `_log_run("t", that_string)`, and assert the resulting `log.md` contains `"... [truncated]"` and does NOT contain the final 100 chars of the input
- [ ] Add a test for `_log_run` nested-directory creation: set `WORKBUDDY_HOME` to `tmp_path / "deep" / "nested" / "wb"` (parent dirs do NOT exist), call `_log_run`, and assert the file at `<that path>/log.md` exists with the entry written
...
- [ ] No production-code changes to `src/workbuddy/cli.py`. If a test reveals a real bug, the Coder may fix it but MUST call it out explicitly in `LOG.md`
- [ ] All existing 9 tests still pass; new tests added bring the total upward without modifying old tests
- [ ] `python -m pytest` passes with no network and no `ANTHROPIC_API_KEY`
