...
## Acceptance
- [x] `pyproject.toml` adds `mcp>=1.0` to `[project] dependencies`. Existing `anthropic>=0.40` and the dev extra stay as-is
...
- [x] Add `--mcp-list-tools` to the existing `mode_group` so it is mutually exclusive with `--exec` and `--git`
- [x] After argparse parses but before the API call: if `args.mcp_list_tools` is True and `args.mcp_server` is None → exit 5 with stderr `error: --mcp-list-tools requires --mcp-server`. If `args.mcp_server` is set but `args.mcp_list_tools` is False → exit 5 with stderr `error: --mcp-server is only meaningful with --mcp-list-tools`
- [x] Add an async helper `async def _async_list_tools(server_argv: list[str]) -> list` that:
...
- [x] Add a sync wrapper `def _run_mcp_list_tools(args) -> int` that:
  ...
  6. Return 0
- [x] Wire `main()`: after argparse and the early `--mcp-list-tools` / `--mcp-server` argument validation, BEFORE the API key check and BEFORE the API call, dispatch: `if args.mcp_list_tools: return _run_mcp_list_tools(args)`. The Anthropic API key is NOT required for this mode (no Claude call). The `task` positional arg is required by argparse but ignored — print a one-line note on stderr `note: --mcp-list-tools mode ignores the task argument` if `args.task` is non-empty
- [x] No log.md or history.jsonl entries for this mode (it's pure inspection — neither user-facing prose nor a confirmable command)
- [x] `tests/test_cli.py` adds (use `monkeypatch.setattr(cli_mod, "_async_list_tools", _fake)` to bypass the real MCP SDK; pytest-asyncio is NOT required because the production code's `asyncio.run` resolves the coroutine):
...
- [x] All 49 existing tests must continue to pass unchanged. The new mcp dep should not affect them
- [x] After the `pyproject.toml` change, the Coder MUST run `pip install -e ".[dev]"` in the venv before pytest, so `mcp` becomes importable. (Note this in LOG.md so the Reviewer knows the venv was updated)
- [x] `python -m pytest` passes (no real MCP server, no network, no `ANTHROPIC_API_KEY` needed for the new tests)
- [x] `README.md` "Usage" section gains a brief `--mcp-list-tools` paragraph after the `--git` paragraph: a one-line `workbuddy --mcp-list-tools --mcp-server "python -m my_mcp_server" .` example, and an explicit note that this mode does NOT execute tools or call Claude — it just inspects the server's advertised capabilities. Mention that a future slice will add tool execution
- [x] Total project Python LOC stays under ~1400. Don't pad
- [x] Append a dedicated BACKLOG annotation: change `- [x] MCP integration` to `- [⏳] MCP integration *(slice 1: --mcp-list-tools listing only; tool execution and resource subscription deferred)*`
